...
- Bridge-hosted manual-login runs reuse the dedicated Oracle browser profile on Windows, but Oracle Chrome is only launched for actual remote runs. Successful runs close it by default while preserving the signed-in profile on disk. If login is required, the browser stays open so you can sign in and rerun. Use `--browser-keep-browser` only for debugging.
